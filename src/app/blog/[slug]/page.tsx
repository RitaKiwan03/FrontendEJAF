    ...
    <>
      <section className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <Reveal className="space-y-4 rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 shadow-[0_20px_80px_rgba(2,6,23,0.35)] backdrop-blur-xl">
          <h1 className="text-4xl font-semibold tracking-tight text-white sm:text-5xl">{copy.page.blogTitle}</h1>
          <ol className="flex flex-wrap items-center gap-2 text-sm text-slate-400">
            <li>
              <Link href={locale === "ar" ? "/?lang=ar" : "/"} className="transition-colors hover:text-white">
                {copy.page.blogBreadcrumbHome}
              </Link>
            </li>
            <li>/</li>
            <li>
              <Link href={locale === "ar" ? "/blog?lang=ar" : "/blog"} className="transition-colors hover:text-white">
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.6fr)_minmax(280px,0.8fr)] lg:items-start">
          <article className="space-y-8 rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 shadow-[0_20px_80px_rgba(2,6,23,0.35)] backdrop-blur-xl sm:p-8">
            ...
            <div className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-slate-950/70">
              <div className="aspect-[16/9] bg-cover bg-center" style={{ backgroundImage: `linear-gradient(180deg, rgba(2,6,23,0.08), rgba(2,6,23,0.82)), url(${post.image})` }} />
            </div>

            <header className="space-y-4">
              <h1 className="text-3xl font-semibold tracking-tight text-white sm:text-5xl">{post.title}</h1>
            ...
            <div className="flex flex-wrap gap-2 border-t border-white/10 pt-6 text-sm text-slate-400">
              {post.tags.map((tag) => (
                <span key={tag} className="rounded-full border border-white/10 bg-slate-950/60 px-3 py-1 text-xs uppercase tracking-[0.18em] text-slate-300">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <aside className="space-y-6">
            ...
            <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-5 shadow-[0_20px_80px_rgba(2,6,23,0.24)] backdrop-blur-xl">
              <h2 className="text-xl font-semibold text-white">{copy.page.blogRelatedPosts}</h2>
              <div className="mt-5 space-y-4">
                {popularPosts
                  .filter((item) => item.slug !== post.slug)
                  .slice(0, 3)
                  .map((item) => (
                    <article key={item.id} className="space-y-2 border-b border-white/10 pb-4 last:border-b-0 last:pb-0">
                      <Link href={`/blog/${item.slug}`} className="block text-base font-medium text-white transition-colors hover:text-cyan-300">
                        {item.title}
                      </Link>
                      <p className="text-sm text-slate-400">{formatLocalizedDate(item.createdAt, locale)}</p>
                    </article>
                  ))}
              </div>
            </div>
            ...
            <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-5 shadow-[0_20px_80px_rgba(2,6,23,0.24)] backdrop-blur-xl">
              <h2 className="text-xl font-semibold text-white">{copy.page.blogMoreReading}</h2>
              <div className="mt-5 space-y-4">
                {popularPosts.slice(0, 2).map((item) => (
                  <Link key={item.id} href={`/blog/${item.slug}`} className="block rounded-2xl border border-white/10 bg-slate-950/55 p-4 text-sm text-slate-300 transition-colors hover:border-cyan-300/20 hover:text-white">
                    {item.title}
                  </Link>
                ))}
              </div>
