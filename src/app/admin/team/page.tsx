...
export default function AdminTeamPage() {
  ...
  return (
    <AdminShell
      ...
      description={
        isAr
          ? "إدارة أعضاء الفريق وهيكله الهرمي"
          : "Manage team members and hierarchy"
      }
    >
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                value={form.role_en}
                onChange={(e) =>
                  setForm((f) => ({ ...f, role_en: e.target.value }))
                }
                placeholder="CEO"
              />
            </div>
            <div>
              ...
              <input
                ...
                value={form.role_ar}
                onChange={(e) =>
                  setForm((f) => ({ ...f, role_ar: e.target.value }))
                }
          ...
          <div className="flex gap-3">
            ...
            {editId && (
              <button
                ...
                className="rounded-full border border-white/10 px-5 py-2.5 text-sm text-slate-400 hover:text-white transition-colors"
              >
                {isAr ? "إلغاء" : "Cancel"}
              </button>
            )}
                    ...
                    >
                      <td className="px-5 py-3">
                        {m.image ? (
                          ...
                          <div className="h-10 w-10 rounded-full bg-slate-800 flex items-center justify-center text-cyan-300 text-sm font-semibold">
                            {m.name_en.charAt(0)}
                          </div>
                        )}
                      </td>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="rounded-xl border border-white/10 px-3 py-1.5 text-xs text-slate-300 hover:text-white hover:border-white/20 transition-colors"
                          >
                            {isAr ? "تعديل" : "Edit"}
                          </button>
                          <button
                            onClick={() => handleDelete(m.id)}
                            className="rounded-xl border border-rose-400/20 px-3 py-1.5 text-xs text-rose-400 hover:bg-rose-400/10 transition-colors"
                          >
